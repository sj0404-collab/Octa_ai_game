...
export class Player {
  ...
  constructor(private readonly scene: Scene, spriteUrl: string) {
    this.mesh = MeshBuilder.CreatePlane("night-shift-worker", { size: 1.5 }, scene);
  ...
  reset() {
    ...
    this.mesh.position.set(this.position.x, 0.42, this.position.z); this.mesh.rotation.y = 0; this.spriteTexture.uOffset = 0;
  ...
  setFirstPerson(enabled: boolean) { const visible = !enabled; this.mesh.setEnabled(visible); this.coreBeacon.setEnabled(visible); this.mesh.isVisible = visible; this.coreBeacon.isVisible = visible; this.trails.forEach((trail) => { trail.mesh.isVisible = visible; }); this.pulseRings.forEach((ring) => { ring.mesh.isVisible = visible; }); }
  setSpawn(position: Point2) { this.position = { ...position }; this.mesh.position.set(position.x, 0.42, position.z); }
  ...
  update(delta: number, direction: Point2, resolveMove?: (from: Point2, wanted: Point2) => Point2) {
    ...
    this.mesh.position.set(this.position.x, (this.crouching ? 0.28 : 0.42) + airborneHeight + Math.sin(performance.now() * 0.006) * 0.03, this.position.z);
  ...
  setFacing(angle: number) { this.facing = angle; this.mesh.rotation.y = angle; }
