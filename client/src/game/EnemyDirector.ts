...
const SPECS: Record<EnemyKind, EnemySpec> = { watcher: { range: 6.35, patrolSpeed: 1.18, chaseSpeed: 2.42, cone: Math.PI / 4.1, color: CORAL, noiseBias: 0.2, faceIndex: 0 }, hunter: { range: 4.85, patrolSpeed: 1.62, chaseSpeed: 3.82, cone: Math.PI / 3.1, color: AMBER, noiseBias: 0.48, faceIndex: 1 }, harvester: { range: 4.2, patrolSpeed: 1.1, chaseSpeed: 2.92, cone: Math.PI / 2.2, color: VIOLET, noiseBias: 0.8, faceIndex: 2 } };

class EnemyAgent {
  readonly mesh: Mesh; readonly face: Mesh; readonly cone: Mesh; readonly signal: Mesh; readonly rigParts: Mesh[] = []; position: Point2; mode: SentinelMode = "patrol";
  ...
  constructor(private readonly scene: Scene, private readonly spawn: EnemySpawn, faceUrl: string) {
    ...
    this.face = MeshBuilder.CreateTorus(`office-pursuer-visor-${spawn.kind}`, { diameter: 0.68, thickness: 0.08, tessellation: 16 }, scene); this.face.rotation.x = Math.PI / 2; this.face.parent = this.mesh; this.face.position = new Vector3(0, 0.52, 0); const faceMaterial = new StandardMaterial(`office-pursuer-visor-material-${spawn.kind}-${spawn.x}`, scene); faceMaterial.emissiveColor = this.spec.color; faceMaterial.disableLighting = true; this.face.material = faceMaterial;
    ...
    this.signal = MeshBuilder.CreateTorus(`office-pursuer-signal-${spawn.kind}-${spawn.x}`, { diameter: 1.32, thickness: 0.055, tessellation: 28 }, scene); this.signal.rotation.x = Math.PI / 2; const signalMaterial = new StandardMaterial(`office-pursuer-signal-material-${spawn.kind}-${spawn.x}`, scene); signalMaterial.emissiveColor = CORAL; signalMaterial.alpha = 0.78; signalMaterial.disableLighting = true; this.signal.material = signalMaterial;
  }
  ...
  update(delta: number, player: Point2, pulseRadius: number, noise: number, flashlightOn: boolean, flashlightDirection: Point2, environment: ReactorEnvironment) {
    ...
    this.steer(target, speed, delta, environment); if (this.mode === "patrol" && distance(this.position, target) < 0.25) this.patrolIndex = (this.patrolIndex + 1) % this.spawn.patrol.length; if (this.mode === "search" && distance(this.position, target) < 0.32) this.memory = 0; this.sync();
  }
  catches(player: Point2) { return this.mode === "chase" && distance(this.position, player) < 0.62; }
  dispose() { this.rigParts.forEach((part) => part.dispose()); this.mesh.dispose(); this.face.dispose(); this.cone.dispose(); this.signal.dispose(); }
  ...
  private steer(target: Point2, speed: number, delta: number, environment: ReactorEnvironment) { const dx = target.x - this.position.x; const dz = target.z - this.position.z; const length = Math.hypot(dx, dz) || 1; const wanted = { x: this.position.x + (dx / length) * speed * delta, z: this.position.z + (dz / length) * speed * delta }; const resolved = environment.resolveMovement(this.position, wanted); if (Math.hypot(resolved.x - this.position.x, resolved.z - this.position.z) > 0.001) this.position = resolved; this.facing = Math.atan2(dx / length, dz / length); }
  private sync() { const clock = performance.now() * 0.006 + this.spawn.x; const bob = Math.sin(clock) * 0.045; this.mesh.position.set(this.position.x, 0.46 + bob, this.position.z); this.mesh.rotation.y = this.facing; this.rigParts.forEach((part, index) => { if (index === 0) part.scaling.x = 1 + Math.sin(clock * 0.5) * 0.04; else part.rotation.z = Math.sin(clock * (this.mode === "chase" ? 3.4 : 1.45) + index) * (this.mode === "chase" ? 0.52 : 0.18); }); this.cone.position.set(this.position.x, 0.025, this.position.z); this.cone.rotation.y = this.facing - Math.PI / 2; this.cone.isVisible = this.mode !== "stunned"; this.signal.position.set(this.position.x, 0.045, this.position.z); this.signal.scaling.setAll(this.mode === "chase" ? 1.25 + Math.sin(clock * 4) * 0.14 : 0.88 + Math.sin(clock * 2.2) * 0.08); this.signal.isVisible = this.mode !== "stunned"; const coneMaterial = this.cone.material as StandardMaterial; coneMaterial.alpha = this.mode === "chase" ? 0.47 : this.mode === "alert" || this.mode === "search" ? 0.31 : 0.16; (this.signal.material as StandardMaterial).alpha = this.mode === "chase" ? 0.96 : 0.56; (this.mesh.material as StandardMaterial).emissiveColor = this.mode === "stunned" ? MINT : this.spec.color.scale(this.mode === "chase" ? 0.78 : 0.4); }
}
